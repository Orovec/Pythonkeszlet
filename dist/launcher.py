...
MAIN_APP_FILE = "launcher.py"


def log_error(message):
    """Hibák rögzítése egy helyi log fájlba, hogy látszódjon a probléma."""
    try:
...
def check_for_updates():
    """Megpróbálja frissíteni a main.py-t, de hiba esetén sem áll meg."""
    try:
        local_version = "1.0.0"
        ...
        # Ha nincs net vagy hiba van, logoljuk, de fut tovább offline módban
        log_error(f"Frissítési hiba (nem kritikus): {e}")


def launch_app():
    """Elindítja a main.py-t a megfelelő Python környezettel."""
    if not os.path.exists(MAIN_APP_FILE):
        root = tk.Tk()
        root.withdraw()
        messagebox.showerror("Kritikus Hiba",
                             "A 'main.py' fájl nem található a launcher mellett, és a letöltés sem sikerült (nincs internet?).")
        sys.exit(1)

    try:
        # Windows alatt a pythonw.exe megakadályozza a felesleges háttér-konzol ablak feltűnését a fő programnál
